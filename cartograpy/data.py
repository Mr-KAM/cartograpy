...
def _print_describe(info: dict):
    ...
    print(f"  Couches  : {info['nb_layers']}")

    if info["type"] == "raster":
        dims = info.get("dimensions", {})
        print(f"  Dimensions : {dims.get('width')} x {dims.get('height')} pixels")
        res = info.get("resolution", {})
        print(f"  Resolution : {res.get('x'):.6f} x {res.get('y'):.6f}")
        print(f"  CRS : {info.get('crs')}")
        b = info.get("bounds", {})
        print(f"  Emprise : [{b.get('left'):.4f}, {b.get('bottom'):.4f}, "
              f"{b.get('right'):.4f}, {b.get('top'):.4f}]")
        print(f"  {dash}")
        for band in info["layers"]:
            print(f"  Bande {band['band']} : dtype={band['dtype']}, "
        ...
        for layer in info["layers"]:
            ...
            print(f"    Entites   : {layer['features']}")
            print(f"    Geometrie : {', '.join(layer['geometry_types']) if layer['geometry_types'] else 'aucune'}")
            print(f"    Colonnes  : {layer['nb_columns']} -> {layer['columns']}")
            print(f"    CRS       : {layer['crs']}")
            b = layer.get("bounds", {})
            if b:
                print(f"    Emprise   : [{b.get('left', 0):.4f}, {b.get('bottom', 0):.4f}, "
                      f"{b.get('right', 0):.4f}, {b.get('top', 0):.4f}]")

    elif info["type"] == "tabular":
        for layer in info["layers"]:
            print(f"  {dash}")
            print(f"  Table : {layer['name']}")
            print(f"    Lignes   : {layer['rows']}")
            print(f"    Colonnes : {layer['columns']} -> {layer['column_names']}")
...
class Gee:
    ...
    def _authenticate(self, service_account: Optional[str], key_file: Optional[str]) -> None:
        ...
        try:
            ...
            print("⚠️  Authentification requise. Lancement de ee.Authenticate()…")
            ee.Authenticate()
            ee.Initialize(project=self.project)
            print("✅ Google Earth Engine initialisé avec succès.")
    ...
    def get_sentinel2(
        ...
        print(f"📡 Sentinel-2 : {size} image(s) trouvée(s) [{start} → {end}]")
    ...
    def get_landsat9(
        ...
        self._last_collection = col
        size = col.size().getInfo()
        print(f"🛰️  Landsat-9 : {size} image(s) trouvée(s) [{start} → {end}]")
    ...
    def get_sentinel1(
        ...
        self._last_collection = col
        size = col.size().getInfo()
        print(f"📻 Sentinel-1 : {size} image(s) trouvée(s) [{start} → {end}]")
    ...
    def collection(
        ...
        self._last_collection = col
        size = col.size().getInfo()
        print(f"📂 Collection «{collection_id}» : {size} image(s)")
        return col
    ...
    def download(
        ...
        print(f"⬇️  Téléchargement vers «{output_path}» (scale={scale}m, crs={crs})…")
    ...
    def export_to_drive(
        ...
        task.start()
        print(f"🚀 Tâche Drive lancée : «{description}» → dossier «{folder}»")
        print("   Suivre avec : ee.batch.Task.list()")
    ...
    def add_layer(
        ...
        print(f"🗺️  Layer «{name}» ajouté à la carte.")
        return m
