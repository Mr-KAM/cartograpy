...
flèche du Nord, grille, export. C'est la classe la plus riche du package (70
méthodes publiques).
...
Référence complète des 70 méthodes de `Map`, groupées par thème :
[Reference mapper](/reference/mapper.qmd).

## Planches multi-panneaux (Layout)

Pour assembler plusieurs `Map` en une seule planche publication (titre,
légende partagée, source, crédits, lettres de panneaux `(a)`, `(b)`...),
utilisez `Layout` plutôt que de composer les images à la main :
...
7 gabarits visuels prêts à l'emploi (`template=`) : `"scientific"`,
`"journal"`, `"thesis"`, `"atlas"`, `"report"`, `"poster"`, `"minimal"`. Pour
une planche à un seul panneau, le raccourci `m.layout("scientific")` évite
d'instancier `Layout` explicitement. Détail complet : [Méthodes de
Layout](/reference/mapper.qmd#méthodes-de-layout).
...
```python
my_cmap = "Acanthostracion_polygonius"
...
if not lux.empty:
    ...
    # position figure de la flèche calculée depuis le centroïde du pays
    # (transData -> transFigure), plutôt qu'une valeur fixe : reste correcte
    # si l'étendue ou la taille de la figure change
    m.fig.canvas.draw()
    lux_point = lux.iloc[0]["centroid"]
    head_x, head_y = m.ax.transData.transform((lux_point.x, lux_point.y))
    head_position = tuple(m.fig.transFigure.inverted().transform((head_x, head_y)))

    m.add_fig_arrow(
        tail_position=(0.32, 0.70),
        head_position=(head_position[0] - 0.01, head_position[1] + 0.01),
        radius=0.3,
...
m.add_image("logo.png", (0.44, 0.14), zoom=0.035)
