...
::: {.callout-tip}
## CRS géographique vs projeté : le piège classique

Sur une couche en `EPSG:4326` (degrés), `vt.buffer(1000)` produit un tampon de
1000 **degrés**, pas 1000 mètres. `buffer()`, `area()` et
`distance_to_nearest()` détectent ce cas automatiquement : si le CRS est
géographique, elles reprojettent temporairement vers l'UTM estimé
(`estimate_utm()`) avant de calculer, puis reviennent au CRS d'origine.

```python
vt = VectorTools(gdf)          # gdf en EPSG:4326
vt.crs_info()                  # {"epsg": 4326, "is_geographic": True, ...}

vt.buffer(1000, unit="m")      # vrai tampon de 1000 m (auto_project=True par défaut)
vt.area(unit="ha").gdf["area"] # surface en hectares, pas en degrés²
vt.distance_to_nearest(routes, unit="km")
```

`auto_project=False` retrouve le comportement historique (calcul direct dans
les unités du CRS courant), utile si la couche est déjà dans un CRS projeté
adapté.
:::
...
| `crs_info()` | `dict` (`epsg`, `name`, `is_geographic`, `unit`) : le CRS courant est-il en degrés ou dans une unité métrique. |
| `estimate_utm()` | CRS UTM estimé à partir du centroïde de l'étendue de la couche (délègue à `GeoDataFrame.estimate_utm_crs()`). |
| `to_local_crs()` | Reprojette vers le CRS UTM estimé (`estimate_utm()` + `reproject_layer()`). |
...
| `buffer(distance, unit="m", auto_project=True)` | Zone tampon autour de chaque géométrie. Si le CRS est géographique (degrés) et `auto_project=True` (défaut), reprojette temporairement vers l'UTM estimé pour interpréter `distance` en `unit` (`"m"` ou `"km"`) plutôt qu'en degrés, puis revient au CRS d'origine. `auto_project=False` retrouve l'ancien comportement (tampon dans les unités brutes du CRS). |
...
| Méthode | Description |
|---|---|
| `area(column_name="area", unit="m2", auto_project=True)` | Ajoute une colonne de surface. Si le CRS est géographique et `auto_project=True` (défaut), la surface est calculée après reprojection temporaire vers l'UTM estimé, en `unit` (`"m2"`, `"ha"` ou `"km2"`) plutôt qu'en degrés². `auto_project=False` retrouve l'ancien comportement. |
...
| Méthode | Description |
|---|---|
| `distance_to_nearest(other, column_name="dist_nearest", unit="m", auto_project=True)` | Distance de chaque entité au plus proche élément d'une autre couche. Si le CRS est géographique et `auto_project=True` (défaut), calculée après reprojection temporaire vers l'UTM estimé, en `unit` (`"m"` ou `"km"`) plutôt qu'en degrés. |
