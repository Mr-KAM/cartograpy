...
::: {.callout-warning}
`source="seaborn"`, `"mplcyberpunk"` et `"scienceplots"` nécessitent les paquets
correspondants installés séparément (seaborn est une dépendance cœur de
cartograpy, mais `mplcyberpunk` et `SciencePlots` sont optionnels et ne font pas
partie des dépendances de `pyproject.toml`).
:::
